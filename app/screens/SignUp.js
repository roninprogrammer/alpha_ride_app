...
import { View, Alert, ScrollView, ActivityIndicator, AsyncStorage } from "react-native";
...
export default class SignUp extends Component {
  ...
  VerifyPhoneNumber = () => {
    ...
    firebase.auth().signInWithPhoneNumber(this.state.phoneNumber)
      .then((confirmResult) => {
        // If this confirmResult comes out, set confirmResult To confirResuls
        // So that, the render can now display where to input the code.
  ...
  SaveDbDetails = () => {
    ...
    var UserData = {
          ...
          // AsyncStore UserData then Save UserData to database
    ...
    if (confirmResult && codeInput.length) {
      confirmResult.confirm(codeInput)
        .then((data) => {
          ...
          //AsyncStore Phone Number

          AsyncStorage.setItem('PhoneNumber', this.state.phoneNumber);

          // We can now save Phone Number to database since its now verified
          let user = firebase.auth().currentUser;
          ...
          dataBase = firebase.firestore()
          dataBase.collection('users').doc(user.uid).update({PhoneNumber: this.state.PhoneNumber})
